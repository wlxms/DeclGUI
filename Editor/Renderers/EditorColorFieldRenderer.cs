...
namespace DeclGUI.Editor.Renderers
{
    ...
    public class EditorColorFieldRenderer : EditorElementRenderer<ColorField>
    {
        ...
        public override void Render(RenderManager mgr, in ColorField element)
        {
            ...
            try
            {
                var style = editorMgr.ApplyStyle(element.Style, EditorStyles.colorField);
        ...
        /// 计算ColorField元素的期望大小
        /// </summary>
        public override Vector2 CalculateSize(RenderManager mgr, in ColorField element, in DeclStyle? style)
        {
            var editorMgr = mgr as EditorRenderManager;
            if (editorMgr == null)
                return Vector2.zero;
