...
namespace DeclGUI.Components
{
    ...
    public struct FoldoutHeaderGroup : IElement<FoldoutHeaderGroupState>, IContainerElement, IEventfulElement, IStylefulElement
    {
        ...
        public FoldoutHeaderGroup WithStyle(IDeclStyle style)
        {
            ...
            newGroup._contextParams = _contextParams;
        ...
        IContainerElement IContainerElement.WithContext(params IContextProvider[] contextParams)
        {
            ...
            newGroup.Key = Key;
            newGroup.Events = Events;
            newGroup._contextParams = contextParams?.Where(c => c != null).ToArray();
            if (_count > 0 && _elements != null)
            {
                for (int i = 0; i < _count; i++)
                {
        ...
        /// <param name="contextParams">上下文参数</param>
        /// <returns>带有上下文参数的新容器实例</returns>
        public FoldoutHeaderGroup WithContext(params IContextProvider[] contextParams)
        {
            var newGroup = new FoldoutHeaderGroup(HeaderElement, InitialExpanded, HeaderStyle, Style);
            newGroup.Key = Key;
            newGroup.Events = Events;
