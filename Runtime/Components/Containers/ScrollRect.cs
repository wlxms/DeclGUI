...
namespace DeclGUI.Components
{
    ...
    public struct ScrollRect : IContainerElement, IEventfulElement, IStylefulElement
    {
        ...
        {
            var newScrollRect = new ScrollRect(_boxSkin, ScrollPosition, OnScroll, AlwaysShowVertical, AlwaysShowHorizontal, Style, GetElementsArray());
            newScrollRect.Key = Key;
            newScrollRect.Events = Events;
            newScrollRect._contextParams = contextParams?.Where(c => c != null).ToArray();
        ...
        /// <param name="contextParams">上下文参数</param>
        /// <returns>带有上下文参数的新容器实例</returns>
        public ScrollRect WithContext(params IContextProvider[] contextParams)
        {
            var newScrollRect = new ScrollRect(_boxSkin, ScrollPosition, OnScroll, AlwaysShowVertical, AlwaysShowHorizontal, Style, GetElementsArray());
            newScrollRect.Key = Key;
            newScrollRect.Events = Events;
