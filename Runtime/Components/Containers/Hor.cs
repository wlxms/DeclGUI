...
namespace DeclGUI.Components
{
    ...
    public struct Hor : IContainerElement, IEventfulElement, IStylefulElement
    {
        ...
        public Hor WithStyle(IDeclStyle style)
        {
            ...
            newHor._contextParams = _contextParams;
        ...
        IContainerElement IContainerElement.WithContext(params IContextProvider[] contextParams)
        {
            ...
            newHor.Key = Key;
            newHor.Events = Events;
            newHor._contextParams = contextParams?.Where(c => c != null).ToArray();
            if (_count > 0 && _elements != null)
            {
                for (int i = 0; i < _count; i++)
                {
        ...
        /// <param name="contextParams">上下文参数</param>
        /// <returns>带有上下文参数的新容器实例</returns>
        public Hor WithContext(params IContextProvider[] contextParams)
        {
            var newHor = new Hor(_boxSkin, Style);
            newHor.Key = Key;
            newHor.Events = Events;
