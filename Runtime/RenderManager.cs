...
namespace DeclGUI.Core
{
    ...
    public abstract class RenderManager
    {
        ...
        public Stack<IElement> RenderStack = new Stack<IElement>();

        public Stack<IElement> CalculateStack = new Stack<IElement>();
        ...
        public virtual void RenderElement(in IElement element)
        {
            if (element == null)
                return;
        ...
        public bool IsRendering<T>() where T : IElement
        {
            return RenderStack.Any(e => e is T);
        }

        public bool IsCalculating<T>() where T : IElement
        {
            return CalculateStack.Any(e => e is T);
        }
        ...
        private void RenderElementInternal(IElement element)
        {
            ...
            try
            {
                ...
                if (isContainer)
                {
                    ...
                    {
                        var contextParams = container.ContextParams ?? Array.Empty<IContextProvider>();
                        // 将上下文参数入栈
                        foreach (var contextParam in contextParams)
                        {
                            if (contextParam is ISpecialContext specialContext)
                            {
                                // 处理特殊上下文
                                var contexts = HandleSpecialContext(specialContext);
                                if (contexts != null && contexts.Length > 0)
                                {
                                    foreach (var ctx in contexts)
                                    {
                                        Type ctxType = ctx.GetType();
                                        ContextStack.Push(ctxType, ctx);
                                    }
                                }
                            }
                            else
                            {
                                // 普通上下文直接入栈
                                Type contextType = contextParam.GetType();
                                ContextStack.Push(contextType, contextParam);
                            }
                        }
                    }
                }
                ...
                try
                {
                    IElement renderTarget = null;
                    if (element is IStatefulElement statefulElement)
                    {
                        renderTarget = statefulElement.Render(state.State);
                    }
                    else
                    {
                        // 如果没有找到渲染器，调用元素的Render方法
                        renderTarget = element.Render();
                    }
            ...
            finally
            {
                ...
                if (isContainer)
                {
                    ...
                    if (element is IContainerElement container)
                    {
                        var contextParams = container.ContextParams;
                        if (contextParams != null && contextParams.Count > 0)
                        {
                            // 按相反顺序弹出上下文参数
                            for (int i = contextParams.Count - 1; i >= 0; i--)
                            {
                                ...
                                if (contextParam is ISpecialContext specialContext)
                                {
                                    ...
                                    if (contexts != null && contexts.Length > 0)
                                    {
                                        for (int j = contexts.Length - 1; j >= 0; j--)
                                        {
                                            Type ctxType = contexts[j].GetType();
                                            ContextStack.Pop(ctxType);
                                        }
                                    }
                                }
                                else
        ...
        {
            // 处理特殊上下文 - 获取其产生的实际上下文
            if (context is ISpecialContext specialContext)
            {
                var contexts = HandleSpecialContext(specialContext);
                if (contexts != null && contexts.Length > 0)
                {
                    ...
                    finally
                    {
                        ...
                        {
                            Type ctxType = contexts[i].GetType();
                            ContextStack.Pop(ctxType);
                        }
                    }
                }
        ...
        {
            return CalculateElementSize(element, parentStyle, null);
        }
        ...
        /// <param name="element">要计算大小的元素</param>
        /// <param name="parentStyle">父元素的样式（可选）</param>
        /// <param name="state">元素的状态，对于IStatefulElement必须提供</param>
        /// <returns>元素的期望大小</returns>
        public Vector2 CalculateElementSize(IElement element, IDeclStyle parentStyle, IElementState state)
        {
            CalculateStack.Push(element);
            bool isPushRootState = false;
            EnsureInitialized();
            if (StateStack.IsEmpty())
            {
                PushRootStateStack();
                isPushRootState = true;
            }
            ...
            try
            {
                ...
                if (element is IContextProvider contextProvider)
                {
                    ...
                    {
                        return CalculateElementSize(contextProvider.Child, parentStyle, state);
                    }
                    return Vector2.zero;
                }
                ...
                if (element is IContextConsumer contextConsumer)
                {
                    ...
                    try
                    {
                        ...
                        {
                            return CalculateElementSize(renderedElement, parentStyle, state);
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                if (element is IStatefulElement statefulElement)
                {
                    if (state == null)
                    {
                        state = GetElementState(statefulElement);
                    }

                    if (state == null)
                    {
                        throw new InvalidOperationException(
                            $"Stateful element {element.GetType().Name} requires state parameter for size calculation. " +
                            $"Please provide the state parameter when calling CalculateElementSize.");
                    }

                    var elementFlation = statefulElement.Render(state.State);
                    if (elementFlation == null || elementFlation == element)
                        return Vector2.zero;

                    return CalculateElementSize(elementFlation, parentStyle, state);
                }
                ...
                // 尝试获取渲染器并计算大小
                return CalculateElementSize(renderTarget, parentStyle, state);
            }
            finally
            {
                if (isPushRootState)
        ...
        /// 处理特殊上下文
        /// </summary>
        /// <param name="specialContext">特殊上下文实例</param>
        /// <returns>特殊上下文产生的实际上下文数组</returns>
        protected virtual IContextProvider[] HandleSpecialContext(ISpecialContext specialContext)
        {
            // 处理Indent和DeIndent特殊上下文
            switch (specialContext)
            {
                case DeclGUI.Components.Indent indent:
                    return HandleIndentContext(indent);
                case DeclGUI.Components.DeIndent deindent:
                    return HandleDeIndentContext(deindent);
                default:
                    return Array.Empty<IContextProvider>();
            }
        }
        ...
        /// 处理Indent上下文
        /// </summary>
        /// <param name="indent">Indent上下文</param>
        /// <returns>产生的缩进上下文数组</returns>
        protected virtual IContextProvider[] HandleIndentContext(DeclGUI.Components.Indent indent)
        {
            // 获取当前缩进上下文，如果没有则使用默认值
            IndentContext currentIndent;
            if (ContextStack.TryGet<IndentContext>(out currentIndent))
            {
                int currentLevel = currentIndent.Level;
                float currentSize = currentIndent.Size;

                // 计算新的缩进级别和大小
                int newLevel = currentLevel + 1;
                float newSize = indent.CustomSize ?? currentSize;

                // 创建新的缩进上下文
                var newIndentContext = new IndentContext(newLevel, newSize);
                return new IContextProvider[] { newIndentContext };
            }
            else
            {
                // 没有缩进上下文，创建第一个缩进级别
                float newSize = indent.CustomSize ?? 20f;
                var newIndentContext = new IndentContext(1, newSize);
                return new IContextProvider[] { newIndentContext };
            }
        }
        ...
        /// 处理DeIndent上下文
        /// </summary>
        /// <param name="deindent">DeIndent上下文</param>
        /// <returns>产生的缩进上下文数组</returns>
        protected virtual IContextProvider[] HandleDeIndentContext(DeclGUI.Components.DeIndent deindent)
        {
            // 获取当前缩进上下文，如果没有则不需要处理
            IndentContext currentIndent;
            if (ContextStack.TryGet<IndentContext>(out currentIndent))
            {
                int currentLevel = currentIndent.Level;
                ...
                // 计算新的缩进级别
                int newLevel = Math.Max(0, currentLevel - deindent.Levels);

                // 如果级别变化，创建新的缩进上下文
                if (newLevel != currentLevel)
                {
                    var newIndentContext = new IndentContext(newLevel, currentIndent.Size);
                    return new IContextProvider[] { newIndentContext };
                }
            }
            // 如果没有缩进上下文或级别没有变化，返回空数组
            return Array.Empty<IContextProvider>();
        }
        ...
        /// 压入根状态栈
        /// </summary>
        private void PushRootStateStack()
        {
            // 确保有根状态管理器
            if (_rootStorage == null)
            {
            ...
            // 压入根状态栈
            StateStack.Push(_rootStorage);
        }

        /// <summary>
        /// 弹出根状态栈
        /// </summary>
        private void PopRootStateStack()
        {
            if (!StateStack.IsEmpty())
            {
                StateStack.Pop();
